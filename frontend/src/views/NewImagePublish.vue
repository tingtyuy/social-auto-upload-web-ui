<template>
  <div class="image-publish">
    ...
    <main class="publish-main">
      ...
      <div class="main-header">
        <div class="header-left">
          <span class="page-title">新图集发布</span>
          <span
        ...
        <div class="header-right">
          ...
          <el-button :icon="Delete" @click="clearImages" class="header-btn">
            清空图片
          </el-button>
        ...
        <div class="config-section">
          ...
          <div class="media-section">
            ...
            <div style="margin-top: 8px;">
              <el-button type="primary" plain icon="el-icon-download" size="small" @click="aiImportDialogVisible = true">
                从AI系统导入
              </el-button>
              <span style="margin-left: 8px; font-size: 12px; color: #909399;">导入Zr.Admin.NET的AI任务结果图</span>
            </div>
          </div>
        </div>
    ...
    <!-- AI Image Import Dialog -->
    <AiImageImportDialog
      v-model="aiImportDialogVisible"
      @imported="handleAiImagesImported"
    />
  </div>
</template>
...
import {
  ...
  Document, FullScreen, MagicStick, Setting, Promotion, UserFilled, Delete
...
import AiImageImportDialog from '@/components/AiImageImportDialog.vue'
...
// Auto-set cover: always sync cover with the first image
watch(() => commonConfig.images, (newImages) => {
  if (newImages && newImages.length > 0) {
    const firstImage = newImages[0]
    // Only update if cover is not set or cover is different from first image
    if (!commonConfig.coverImage || commonConfig.coverImage.id !== firstImage.id) {
      commonConfig.coverImage = {
        id: firstImage.id,
        name: firstImage.name,
        url: firstImage.url,
        stored_path: firstImage.stored_path,
        size: firstImage.size,
        type: firstImage.type,
      }
    }
  } else {
    commonConfig.coverImage = null
  }
}, { deep: true })
...
// AI Image Import - 直接添加到发布列表，第一张同时设为封面
function handleAiImagesImported(images) {
  if (!images || images.length === 0) return
  const newImages = images.map(img => ({
    id: img.id,
    name: img.name || 'ai_import.png',
    url: img.url,
    stored_path: '',
    size: 0,
    type: 'image',
  ...
  // 通过 .value 访问 computed 的内部响应式对象
  const target = currentEditTarget.value
  target.images = [...(target.images || []), ...newImages]
  // 第一张同时设为封面
  if (!target.coverImage && newImages.length > 0) {
    target.coverImage = { ...newImages[0] }
  }
  ElMessage.success(`已添加 ${newImages.length} 张AI结果图到发布列表`)
}
...
function clearImages() {
  ...
  for (const key of Object.keys(platformOverrides)) {
    platformOverrides[key].images = []
    platformOverrides[key].coverImage = null
  }
  for (const key of Object.keys(accountOverrides)) {
    accountOverrides[key].images = []
    accountOverrides[key].coverImage = null
  }
}
